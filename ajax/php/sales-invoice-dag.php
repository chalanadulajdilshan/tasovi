...
if (isset($_POST['create'])) {
    ...
    foreach ($items as $item) {
        ...
        $final_cost += $item_total; // Assuming cost same as total for DAG



        $DAG_ITEM = new DagItem($item['dag_item_id']);
        $DAG_ITEM->casing_cost = $price;
        $DAG_ITEM->total_amount = $item_total;
        $DAG_ITEM->update();
    }

    //update dag print status
    ...
    if ($invoice_id) {
        foreach ($items as $item) {
            ...
            $AUDIT_LOG->description = 'CREATE DAG INVOICE #' . $invoice_no;
