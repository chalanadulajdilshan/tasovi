jQuery(document).ready(function () {
  ...
  $("#add_cheque").on("click", function () {
    ...
    const cheque_total =
      parseFloat($("#cheque_total").val().replace(/,/g, "")) || 0;
    const outstanding =
      parseFloat($("#outstanding").val().replace(/,/g, "")) || 0;
    ...
    if (
      parseFloat(cheque_total) > parseFloat(outstanding) ||
      parseFloat(amount) > parseFloat(outstanding)
    ) {
      return swal(
        "Exceeded Outstanding",
        "You added more than the Outstanding Amount.",
        "error"
      );
    }
  ...
  $(document).on("input", ".cheque-pay", function () {
    ...
    if (inputVal > overdue || inputVal > remainingBalance) {
      ...
      swal({
        ...
        text: `You can't enter more than Overdue (Rs. ${overdueText}) or Remaining Cheque Balance (Rs. ${remainingBalance.toFixed(
          2
        )})`,
        type: "error",
        timer: 3000,
  ...
  $("#utilize").on("click", function (e) {
    ...
    if (chequeBalance > 0 || cashBalance > 0) {
      swal(
        "Unused Balance Detected",
        "Please utilize the remaining cheque or cash balance before proceeding.",
        "info"
      );
    }
    ...
    function showNoPaymentData() {
      ...
      swal(
        "No Payments",
        "There is no payment data to show in the summary.",
        "warning"
      );
    }
  ...
  $("#create").click(function (event) {
    ...
    if (!$("#code").val() || $("#code").val().length === 0) {
    ...
    } else if (!$("#entry_date").val() || $("#entry_date").val().length === 0) {
      swal({
        title: "Error!",
      ...
      !$("#grandTotal").val() ||
      $("#grandTotal").val().length === 0 ||
      isNaN($("#grandTotal").val())
    ) {
      swal({
        title: "Error!",
      ...
      $.ajax({
        url: "ajax/php/payment-receipt.php", // Adjust this path if needed
        type: "POST",
        data: formData,
        async: false,
        cache: false,
